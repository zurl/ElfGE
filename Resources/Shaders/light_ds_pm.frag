...
vec3 CalcDirLight(vec2 texCoords, DirLight light, vec3 normal, vec3 viewDir);
vec3 CalcPointLight(vec2 texCoords, PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir);
vec3 CalcSpotLight(vec2 texCoords, SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir);
...
//    if(texCoords.y < 0.0)texCoords.y = 0.0;
    ...
    for(int i = 0; i < NR_DIR_LIGHTS; i++){
        result += CalcDirLight(texCoords, dirLight[i], norm, viewDir);
    }
    for(int i = 0; i < NR_POINT_LIGHTS; i++){
        result += CalcPointLight(texCoords  , pointLight[i], norm, FragPos, viewDir);
