...
void GrassRenderer::prepare() {
   ...
   //printf("hello");
    ...
    shader->use();
    dumpGLErrorLog(__LINE__);
    shader->setFloat("time",(float)time*0.000000000001);
    time+= fmod(time+Utility::deltaTime,20000000000);
//    time+=Utility::deltaTime;
    dumpGLErrorLog(__LINE__);
    Transform cameraT = Runtime::getCamera()->getGameObject()->transform;
    shader->setVec3("camDir",cameraT.getPosition());
    dumpGLErrorLog(__LINE__);
    glm::vec2 v1,v2;
    v1.x = cameraT.getPosition().x;
    v1.y = cameraT.getPosition().z;
    v2.x = cameraT.getForward().x;
    v2.y = cameraT.getForward().z;
